...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            ...
            val endoPaginationReached = responseData.listStory.isEmpty()

            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.remoteKeysDao().deleteRemoteKeys()
                    database.databseDao().deleteAll()
                }
                val prevKey = if (page == 1) null else page - 1
                val nextKey = if (endoPaginationReached == true) null else page + 1

                val keys = responseData.listStory.map {
                    it.let { it1 -> RemoteKeys(id = it1.id, prevKey = prevKey, nextKey = nextKey) }

                }
                val listoryItem = responseData.listStory.map {
                    it.let { it1 ->
                        ListStoryItem(
                            it1.id,
                            it.name,
                ...
                database.databseDao().insertStory(listoryItem)
            }
            return MediatorResult.Success(endOfPaginationReached = endoPaginationReached)
